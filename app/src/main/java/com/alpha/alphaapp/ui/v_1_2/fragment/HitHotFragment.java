...
public class HitHotFragment extends AbsMyBaseFragment<ForumBean> {
    @Override
    public int getLayoutId() {
        return R.layout.fragment_hithot;
    }

    @Override
    public int getRecyclerView() {
        return R.id.frag_hithot_rv;
    }

    @Override
    public int getSwipeRefreshLayout() {
        return  R.id.frag_hithot_refresh_layout;
    }
    ...
    protected List<Cell> getCells(List<ForumBean> list) {
        ...
        for (int i=0;i<list.size(); i++){
            cells.add(new HitHotCell( list.get(i)));
        }
        return cells;
    }
