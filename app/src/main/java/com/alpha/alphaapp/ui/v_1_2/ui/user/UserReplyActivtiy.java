...
public class UserReplyActivtiy extends AbsBaseListActivity {
    ...
    public RecyclerView initRecylerView(View view) {
        RecyclerView rv = (RecyclerView) findViewById(R.id.acty_user_reply_rv);
    ...
    public SwipeRefreshLayout initSwipeRefreshLayout(View view) {
        SwipeRefreshLayout sw = (SwipeRefreshLayout) findViewById(R.id.acty_user_reply_sw);
        return sw;
    }
    ...
    public List<Cell> getCells(List list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            cells.add(new UserReplyCell("add"));
        }
        return cells;
    }
    ...
    public static void actionStart(Context context, String uid) {
        Intent intent = new Intent(context, UserReplyActivtiy.class);
